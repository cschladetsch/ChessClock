namespace UIWidgets
{
		...
		where TNotification : NotificationCustom<TNotification>
	{
		...
		protected class ButtonsPool : DialogButtonsPoolCustom<TNotification, ButtonInstance>
		{
			...
			public ButtonsPool(TNotification owner, ReadOnlyCollection<Button> templates, List<ButtonInstance> active, List<List<ButtonInstance>> cache)
		...
		protected ButtonsPool Buttons
		{
			get
			{
				if (buttonsPool == null)
				{
					buttonsPool = new ButtonsPool(this as TNotification, ButtonsTemplates, ButtonsActive, ButtonsCached);
				}
